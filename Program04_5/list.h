...
// serves as the nodes of the list
typedef struct node
{
	int soc_sec_num;		// was id
	char* name;
	struct node*  next;	// recursive struct so need to use struct in component type because node not official type to be used by itself yet
} node;
...
// return NULL if no such node existis in the list
